...
void vtkSlicerSkeletalRepresentationRefinerLogic::Refine(double stepSize, double endCriterion, int maxIter, int interpolationLevel)
{
    ...
    // Refine up spokes
    mUpSpokes = RefinePartOfSpokes(up, stepSize, endCriterion, maxIter, "Refined up spokes");

    // Refine down spokes
    mDownSpokes = RefinePartOfSpokes(down, stepSize, endCriterion, maxIter, "Refined down spokes");
    ...
    ShowImpliedBoundary(interpolationLevel, newHeaderFileName, "Implied boundary after the refinement");
}
                                                                      ...
                                                                      const string &title)
{
    ...
    if(!title.empty())
        Visualize(impliedBoundary, title, 1, 1, 0);
}
                                                               ...
                                                               )
{
    ...
    vtkSmartPointer<vtkAppendPolyData> appendFilter =
      vtkSmartPointer<vtkAppendPolyData>::New();
    size_t numPerRow = (nCols - 1) * (steps.size() - 1) + 1;
...
std::vector<vtkSpoke*>& vtkSlicerSkeletalRepresentationRefinerLogic::RefinePartOfSpokes(const string &srepFileName,
                                                                                        double stepSize,
                                                                                        double endCriterion,
                                                                                        int maxIter,
                                                                                        const string& targetName)
{
    ...
    if(!targetName.empty()){
        Visualize(refinedSrep, targetName, 0, 1, 1);
    }
                                                                     ...
                                                                     std::vector<vtkSpoke *> &output)
{
    ...
    // bot row
    for(size_t i = input.size()-1; i >= (nRows-2)*2+nCols; --i) {
        output.push_back(input[i]);
    }
