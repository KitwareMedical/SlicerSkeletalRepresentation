...
class Spoke(object):
    def __init__(self, radius=None, direction=None,
        ...
        self.ext = np.inf       # extension (i.e., to the medial axis of between-object)
    ...
    def avg(self, another):
        ...
        return Spoke(avg_r, avg_U, None, None)
    def visualize(self):
        ...
        link = None
        if self.ext < np.inf and self.ext > 0:
            # show links outside
            link = vtk.vtkPolyData()
            link_pts = vtk.vtkPoints()
            link_lines = vtk.vtkCellArray()
            arrow = vtk.vtkLine()

            id_ext0 = link_pts.InsertNextPoint(bdry_pt)

            if self.ext_pt is not None:
                id_ext1 = link_pts.InsertNextPoint(self.ext_pt)
            else:
                id_ext1 = link_pts.InsertNextPoint(bdry_pt + (self.ext) * self.U)
            arrow.GetPointIds().SetId(0, id_ext0)
            arrow.GetPointIds().SetId(1, id_ext1)
            link_lines.InsertNextCell(arrow)
            link.SetPoints(link_pts)
            link.SetLines(link_lines)
            named_colors = vtk.vtkNamedColors()
            colors = vtk.vtkUnsignedCharArray()
            colors.SetNumberOfComponents(3)
            colors.InsertNextTypedTuple(named_colors.GetColor3ub("Red"))

            link.GetCellData().SetScalars(colors)

        return spoke_polydata, link
