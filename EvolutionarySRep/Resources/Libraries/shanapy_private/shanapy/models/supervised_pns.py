...
from shanapy.simulations import toy_example_on_s2 as toyS2
...
class SPNS(object):
    ...
    def __init__(self, data=None, itype=9, alpha=0.05, y=None):
        ...
        Args:
            ...
            y (numpy.ndarray or None): class labels
    ...
    def get_subsphere(self, data, small_circle=False):
        ...
        Returns:
            ...
            r (float): the polar angle of the fitted subsphere
        """
        ...
        c0 = initial_center
        _, center, r = est_subsphere(data, c0)

#         sphere = Hypersphere(dim=2)
#         fig = plt.figure(figsize=(8, 8))
#         ax = visualization.plot(data[:, :50].T, space='S2', color='black', alpha=0.7, label='Group 1')
#         ax = visualization.plot(data[:, 50:].T, space='S2', color='red', alpha=0.7, label='Group 2')
#         ax = visualization.plot(center.squeeze(), space='S2', label='Center after rot')
# #        ax = visualization.plot(new_center.squeeze(), space='S2', label='Center before rot')
...
#         plt.show()
    ...
    def least_square_fit_sphere(self, data, initial_center=None, small_circle=False):
        ...
        # ## dir_vec is the normal vector of the decision boundary
        # dir_vec, intercept, slack_variables, di, problem = self.socp(data.T, self.y)

        # slope = -dir_vec[0] / dir_vec[1]
        ...
        # dwd = DWD().fit(data.T, self.y)
        # Z = dwd.predict(np.c_[xx.ravel(), yy.ravel()]).reshape(xx.shape)
        # plt.contourf(xx, yy, Z, cmap=plt.cm.coolwarm, alpha=0.8)
        # plt.scatter(data[0, :], data[1, :], c=self.y, cmap=plt.cm.coolwarm)
        # ABLine2D(*clf2D_slope_intercept(coef=dir_vec, intercept=intercept),
        #          color='black',
        #          label='DWD separating hyperplane')
        # #######
        # intercept = - intercept / dir_vec[1]
        # num_samples = data.shape[1]
        # ### The mean is the point on the decision boundary and minimizes overall distances
        # mean_x = (np.sum(data) / num_samples - intercept) / (slope + 1)
        # mean_y = mean_x * slope + intercept

        # center = np.array([mean_x, mean_y])
        # plt.plot(mean_x, mean_y, 'k^', label="Center")
        # plt.legend()

        # ### SVM classification
        # plt.subplot(1, 2, 2)
        # svc = svm.SVC(kernel='linear', C=1.0).fit(data.T, self.y)
    ...
    def socp(self, X, y, C=1.0, solver_kws={}):
        """
        Second-order-cone programing optimize the distance-weighted margin from DWD
        Parameters
        ----------
...
if __name__ == "__main__":
    ## Generate 2 classes of data on S^2
    num_pos = num_neg = 50
    data = toyS2.simulate_data_on_small_circle()
    data2 = toyS2.simulate_data_on_small_circle(magnifier=-0.5, shift=1.6)
    y = np.array([1] * num_pos + [-1] * num_neg)
    concate_data = np.concatenate((data.T, data2.T))
    spns_model = SPNS(concate_data.T, y=y)
    spns_model.fit()
    spns_resmat, SPNS_coords = spns_model.output
