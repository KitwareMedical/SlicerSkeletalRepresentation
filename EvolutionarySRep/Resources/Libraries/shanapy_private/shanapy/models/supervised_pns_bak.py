"""
This file implements principal nested spheres that take into account class labels
"""
import numpy as np
import scipy
...
class SPNS(object):
    ...
    def __init__(self, data=None, itype=9, alpha=0.05, y=None):
        """
        ...
        self.alpha = alpha
        self.y = y
        self.affinity = np.eye(self.data.shape[1])
    ...
    def get_subsphere(self, data, small_circle=False):
        ...
        def select_supervised_direction(loading_matrix, data):
            """
            Select the best direction that PNS wants to point to North pole (NP)
            Input: loading matrix resulting from SVD of the data
            Input: data that is used to compute the direction of two classes d x n matrix
            
            """
            ## for unsupervised, the direction which has the least variation should point to NP
            c0 = loading_matrix[:, -1]
            if self.y is None:
                return c0

            ## for supervised, the direction that points to NP and the tangent plane of the
            ## projection should be the direction orthogonal to the direction of the two classes
            pos_data = data[:, self.y == 1]
            neg_data = data[:, self.y == 0]
            mean_pos = np.mean(pos_data, axis=1)
            mean_neg = np.mean(neg_data, axis=1)

            mean_diff = mean_pos - mean_neg
            mean_diff /= np.linalg.norm(mean_diff)

            ## find the most discrepent direction in loading matrix
            min_cos_theta = 2
            ret_dir = None
            cand_i_v = 0
            for i_v in range(loading_matrix.shape[1]):
                temp_v = loading_matrix[:, i_v]
                curr_cos = abs(np.dot(mean_diff, temp_v))
                if curr_cos == min_cos_theta and i_v > cand_i_v:
                    cand_i_v = i_v
                    ret_dir = temp_v
                    min_cos_theta = curr_cos
                elif curr_cos < min_cos_theta:
                    cand_i_v = i_v
                    ret_dir = temp_v
                    min_cos_theta = curr_cos
            return ret_dir
        def map_center_point(new_center_tp, rot):
            if new_center_tp is None: return None
        ...
        def est_subsphere(data, c0):
            ...
            while err > tol:
                ...
                new_center_tp, r, super_center = self.least_square_fit_sphere(tp_data, np.zeros(d-1), small_circle)
#                if super_center is not None: new_center_tp = super_center
                if r > np.pi:
                    r = np.pi / 2
                ...
                center = map_center_point(new_center_tp, rot)
                super_center = map_center_point(super_center, rot)
                g_next = obj_fun(center, r, data)
                err = abs(g_now - g_next)
                g_now = g_next
                c0 = center.squeeze() if super_center is None else super_center.squeeze()
                cnt += 1
                if cnt > 30:
        ...
        #c0 = initial_center
        c0 = select_supervised_direction(u, data)
...
#        c0 = initial_center
    ...
    def least_square_fit_sphere(self, data, initial_center=None, small_circle=False):
        ...
        Returns:
            center (numpy.ndarray): the vector of the center of the fitted subsphere
            r (float): the radius of the fitted subsphere
            supervised_center of the two classes
        ...
        def compute_residuals(x):
            ...
            di = di - r
            return di
        def compute_affinity_matrix(dist):
            n, n = dist.shape
            affinity = np.eye(n)
            v_w = 5
            v_b = 5
            for i in range(n):
                for j in range(i):
                    tmp1 = np.argsort(dist[i, self.y==self.y[i]])[v_w]
                    tmp2 = np.argsort(dist[j, self.y==self.y[j]])[v_w]
                    g_w = np.int((self.y[i] == self.y[j]) and dist[i, j] < np.maximum(tmp1, tmp2))
                    tmp1 = np.argsort(dist[i, self.y!=self.y[i]])[v_b-1]
                    tmp2 = np.argsort(dist[j, self.y!=self.y[j]])[v_b-1]
                    g_b = np.int((self.y[i] != self.y[j]) and (dist[i,j]<np.maximum(tmp1, tmp2)))
                    affinity[i,j] = g_w - g_b
                    affinity[j,i]= affinity[i,j]
            return affinity
        def compute_supervised_residuals(x):
            x = x[:, np.newaxis]
            di = np.sqrt(np.sum((data - x) ** 2, axis=0))
            r = np.pi / 2
            ...
            # x = x[:, np.newaxis]
            # centered_data = data - x
            # dist_mat = np.matmul(centered_data.T, centered_data)
            # affinity = compute_affinity_matrix(dist_mat)
            # weighted_gram = centered_data @ affinity @ centered_data.T
            # di = (np.sum(weighted_gram, axis=0))
            # r = np.pi / 2
        ...
        supervised_center = None
        if self.y is not None:
            super_opt = opt.least_squares(compute_supervised_residuals, initial_center, method='lm', max_nfev=50, xtol=1e-15)
            supervised_center = super_opt.x
            center = supervised_center
        if small_circle:
            r = np.mean(di)
        else:
            r = np.pi / 2
        return center, r, supervised_center
