...
def compute_moment_shape(output_folder, bend_list, twist_list, moment_list):
    ...
    for i in range(len(bend_list)):
        bend_mu = bend_list[i]
        twist_mu = twist_list[i]
        moment_name = moment_list[i]
        final_mesh_names = []
        ...
        srep_writer.Update()
def compute_heatmap(output_folder, moment_list):
    std_mesh_path = os.path.join(output_folder, 'final_mesh', 'std_ell_label_SPHARM.vtk')
    ell_meshes, ell_sreps = construct_two_standard_ellipsoids(std_mesh_path)
    variation_modes_folder = 'population_variation'
    output_heatmap_folder = os.path.join(output_folder, variation_modes_folder)
    for moment_name in moment_list:
...
if __name__ == '__main__':
    population_folder = '/playpen/workspace/Simulate_Shapes/simulation_for_links/population_a/'
    m_name = 'sigma'
    bend_top = 0.15 + 0.1/ 3.0
    bend_minus_sigma = -0.1/3.0 + 0.15
    bend_mean = 0.15
    moment_names = ['sigma', 'minus_sigma', 'mean']
    moment_bends = [[0.1 / 3.0, 0.15 + 0.1 / 3.0], [-0.1 / 3.0, bend_minus_sigma],
                    [0, 0.15]]
    compute_moment_shape(population_folder, moment_bends, [[0, 0], [0, 0], [0, 0]], moment_names)
    compute_heatmap(population_folder, moment_names)

    population_folder = '/playpen/workspace/Simulate_Shapes/simulation_for_links/population_b/'
    bend = [[0.1/3.0, 0.01 + 0.1/3.0], [-0.1/3.0, 0.01 - 0.1/3.0], [0.0, 0.01]]
    twist = [[0, 0], [0, 0], [0.0, 0.0]]
    compute_moment_shape(population_folder, bend, twist, moment_names)
    compute_heatmap(population_folder, moment_names)
