...
Adapted for Nick Tapp-Hughes' s-reps in June 2023.
Used as part of RefinerWrapper which works for Nick's sreps.
"""
...
class Refiner:
    ...
    def compute_srad_penalty(self, srep, spoke_offset=0):
        ...
        fold_pt_mat = np.zeros((3, num_crest_pt)) # num_crest_pt
        fold_vec_mat = np.zeros((3, num_crest_pt))
        fold_rad_mat = np.zeros((1, num_crest_pt))
    ...
    def refine(self, srep, num_crest_points=24):
        ...
        refined_srep_poly = self.update_srep(minimizer, base_array, relocate_fold=False)
        # new_loss = obj_func(minimizer)
        
        return refined_srep_poly

    def refine_spoke_lengths_to_boundary(self, srep):
        """
        The main entry of the refinement
        Input: an initial s-rep  srep
        Input: the boundary mesh input_mesh
        """
        print('Refining spoke lengths to boundary...')
