"""
Wraps refiner.py (Zhiyuan's refiner) so that it can be used with Nick's sreps.
"""
import vtk
import numpy as np
...
class RefinerWrapper:
    ...
    def __init__(self, surface_mesh):
        self.refiner = Refiner(surface_mesh)
    
    def refine(self, srep, num_crest_points=24):
        # convert nick srep to zhiyuan srep
        z_srep, mapping = self.n_to_z(srep.polydata)

        # refine zhiyuan srep (length, orthogonalization, srad)
        refined_z_srep = self.refiner.refine(z_srep)

        # refine zhiyuan srep again (just length)
        refined_z_srep = self.refiner.refine_spoke_lengths_to_boundary(refined_z_srep)

        # convert zhiyuan srep back to nick srep
        refined_n_srep = self.z_to_n(refined_z_srep, mapping)

        return refined_n_srep

    def z_to_n(self, z_srep, mapping):
        reordered_srep = z_srep
    ...
    def n_to_z(self, n_srep):

        srep_poly = n_srep

        def reorderOnOneSide(offset=0):
            """
            re-orders the top non-fold non-crest srep points or the bottom ones    
            """
