...
#ifndef SRepInterpolater_H
#define SRepInterpolater_H

class Spoke;
class SRepInterpolater
{
public:
    SRepInterpolater();
    ...
    void Interpolate(double u, double v, Spoke** conerSpokes, Spoke* interpolatedSpoke);
    ...
    // Return: error code
    void InterpolateQuad(Spoke** cornerSpokes, double u, double v, double lambda, Spoke* interpolatedSpoke);
    ...
    void InterpolateSegment(Spoke** endSpokes, double dist, double lambda, Spoke* interpolatedSpoke);


    // Interpolate the spoke between start spoke (startS) and end spoke(endS) given the distance (d) from
    // the start spoke to the end.
    bool InterpolateMiddleSpoke(Spoke* startS, Spoke* endS, double d, Spoke* interpolatedSpoke);

    // interpolate skeletal point
    // Input corner spokes with radius, direction and base point
    void InterpolateSkeletalPoint(Spoke** cornerSpokes, double u, double v, double *output);
