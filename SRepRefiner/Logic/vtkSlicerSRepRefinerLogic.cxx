...
#include "Spoke.h"
#include "newuoa.h"
#include "PolyData2ImageData.h"
#include "ApproximateSignedDistanceMap.h"
#include "GradientDistanceFilter.h"
...
void vtkSlicerSRepRefinerLogic::SetSrepFileName(const std::string &srepFilePath)
{
    ...
    SRep *srep = new SRep(nRows, nCols, up_radii, up_dirs, up_skeletalPoints);
    if(srep->IsEmpty())
    {
        std::cerr << "The s-rep model is empty." << std::endl;
        delete srep;
    ...
    SRep *downSrep = new SRep(nRows, nCols, down_radii, down_dirs, down_skeletalPoints);
    if(downSrep->IsEmpty())
    {
        std::cerr << "The s-rep model is empty." << std::endl;
        delete downSrep;
    ...
    std::vector<Spoke*> crestSpokes, reorderedCrest;
                                                                  ...
                                                                  std::string& srepFileName)
{
    ...
    std::vector<Spoke*> interpolatedSpokes, upSpokes, downSpokes, crestSpokes;
    ...
    std::vector<Spoke*> borderUpSpoke, borderDownSpoke, repeatedInterp;
...
void vtkSlicerSRepRefinerLogic::InterpolateSrep(int interpolationLevel, int nRows, int nCols,
                                                                  ...
                                                                  std::vector<Spoke*> &interpolatedSpokes)
{
    std::vector<Spoke *> interpolatedCrestSpokes;
                                                                  ...
                                                                  std::vector<Spoke *> &interpolatedCrestSpokes)
{
    ...
    SRep *srep = new SRep(nRows, nCols, radiiUp, dirsUp, skeletalPointsUp);
    if(srep->IsEmpty())
    {
        std::cerr << "The s-rep model is empty." << std::endl;
        delete srep;
    ...
    SRepInterpolater interpolater;

    int shares = static_cast<int>(pow(2, interpolationLevel));
    double interval = static_cast<double>(1.0/ shares);
    std::vector<double> steps;
    std::vector<Spoke *> repeatedInterpInterior;
    for(int i = 0; i <= shares; ++i)
    {
    ...
    std::vector<Spoke*> crestSpokes, topCrest;
    ParseCrest(crestFileName, crestSpokes);

    std::vector<Spoke *> tempSpokes;
...
double vtkSlicerSRepRefinerLogic::EvaluateObjectiveFunction(double *coeff)
{
    ...
    for(int i = 0; i < spokeNum; ++i)
    {
        int r = i / num_cols;
        int c = i % num_cols;
        Spoke *thisSpoke = tempSrep->GetSpoke(r, c);
...
void vtkSlicerSRepRefinerLogic::AntiAliasSignedDistanceMap(const std::string &meshFileName)
{
    ...
    PolyData2ImageData polyDataConverter;
                                                                      ...
                                                                      const string &srepFileName)
{
    ...
    vtkSmartPointer<vtkPolyData> southPolyData = vtkSmartPointer<vtkPolyData>::New();
    vtkSmartPointer<vtkPolyData> crestPolyData = vtkSmartPointer<vtkPolyData>::New();
    std::vector<Spoke*> borderUpSpoke, borderDownSpoke, interpolatedCrestSpokes, repeatedInterpUp, repeatedInterpDown;
    ConnectImpliedBoundaryPts(interpolationLevel, nRows, nCols, up,
...
void vtkSlicerSRepRefinerLogic::ConvertSpokes2PolyData(std::vector<Spoke *>input, vtkPolyData *output)
{
...
void vtkSlicerSRepRefinerLogic::SaveSpokes2Vtp(std::vector<Spoke *> input, const string &path)
{
    vtkSmartPointer<vtkPoints> pts = vtkSmartPointer<vtkPoints>::New();
...
void vtkSlicerSRepRefinerLogic::TransSpokes2PolyData(std::vector<Spoke *>input, vtkPolyData *output)
{
    vtkSmartPointer<vtkPoints> pts = vtkSmartPointer<vtkPoints>::New();
    vtkSmartPointer<vtkCellArray> arrows = vtkSmartPointer<vtkCellArray>::New();
...
double vtkSlicerSRepRefinerLogic::ComputeDistance(Spoke *theSpoke, double *normalMatch)
{
...
void vtkSlicerSRepRefinerLogic::TransformSrep2ImageCS(SRep *input, double mat4x4[][4])
{
    if(input->IsEmpty())
    {
        mat4x4 = nullptr;
    ...
    std::vector<Spoke *> spokes = input->GetAllSpokes();
                                                               ...
                                                               )
{
    ...
    SRep *srep = new SRep(nRows, nCols, radii, dirs, skeletalPoints);
    if(srep->IsEmpty())
    {
        std::cerr << "The s-rep model is empty." << std::endl;
        delete srep;
    ...
    std::vector<Spoke *> pSpokes = srep->GetAllSpokes();
    for (size_t i = 0; i < pSpokes.size(); ++i) {
        Spoke *s = new Spoke(*pSpokes[i]);
        primary.push_back(s);
    }
    vtkSmartPointer<vtkPoints> pts = vtkSmartPointer<vtkPoints>::New();
...
void vtkSlicerSRepRefinerLogic::ConnectImpliedCrest(int interpolationLevel,
                                                                      ...
                                                                      std::vector<Spoke*> &upSpokes,
                                                                      std::vector<Spoke*> &downSpokes,
                                                                      vtkPolyData* crestPoly)
{
    std::vector<Spoke*> crestSpokes, topCrest;
    ParseCrest(crest, crestSpokes);

    std::vector<Spoke *> upInterpSpokes, downInterpSpokes, crestInterpSpokes, tempInterp, reorderedCrest;
...
void vtkSlicerSRepRefinerLogic::ConnectFoldCurve(const std::vector<Spoke *> &edgeSpokes,
...
std::vector<Spoke*>& vtkSlicerSRepRefinerLogic::RefinePartOfSpokes(const string &srepFileName, double stepSize, double endCriterion, int maxIter)
{
    mCoeffArray.clear();
    std::vector<double> radii, dirs, skeletalPoints;
    Parse(srepFileName, mCoeffArray, radii, dirs, skeletalPoints);

    SRep *srep = new SRep(mNumRows, mNumCols, radii, dirs, skeletalPoints);
    if(srep->IsEmpty())
    {
        std::cerr << "The s-rep model is empty." << std::endl;
        delete srep;
        srep = nullptr;
        std::vector<Spoke*> emptyRet;
        return emptyRet;
    }
...
double vtkSlicerSRepRefinerLogic::TotalDistOfLeftTopSpoke(SRep *tempSrep,
                                                                            double u, double v,
                                                                            int r, int c,
                                                                            double *normalMatch)
{
...
double vtkSlicerSRepRefinerLogic::TotalDistOfRightTopSpoke(SRep *tempSrep,
                                                                             double u, double v,
                                                                             int r, int c,
                                                                             double *normalMatch)
{
...
double vtkSlicerSRepRefinerLogic::TotalDistOfLeftBotSpoke(SRep *tempSrep,
                                                                            double u, double v,
                                                                            int r, int c,
                                                                            double *normalMatch)
{
...
double vtkSlicerSRepRefinerLogic::TotalDistOfRightBotSpoke(SRep *tempSrep,
                                                                             double u, double v,
                                                                             int r, int c,
                                                                             double *normalMatch)
{
...
double vtkSlicerSRepRefinerLogic::ComputeRSradPenalty(SRep *input)
{
    double penalty = 0.0;
    // Interpolate
    if(input->IsEmpty())
    ...
    std::vector<Spoke*> interpolatedSpokes;
    std::vector<Spoke*> primarySpokes;
    for(int r = 0; r < nCrestPoints; ++r)
    {
        for(int c = 0; c < numCols; ++c)
        {
            Spoke* thisSpoke = input->GetSpoke(r, c);

            std::vector<Spoke*> neighborU, neighborV;
            if(r == 0 && c == 0)
            {
                // top left corner
...
void vtkSlicerSRepRefinerLogic::FindTopRightNeigbors(int r, int c, SRep *input, std::vector<Spoke *> &neighborU, std::vector<Spoke *> &neighborV)
{
    SRepInterpolater interpolater;
    int nRows = input->GetNumRows();
    int nCols = input->GetNumCols();
...
void vtkSlicerSRepRefinerLogic::InterpolateCrest(std::vector<Spoke *> &crestSpoke,
                                                                   std::vector<Spoke *> &interiorSpokes,
                                                                   int interpolationLevel, int nRows,
                                                                   int nCols, std::vector<Spoke*> &crest,
                                                                   std::vector<Spoke*> &interior)
{
...
void vtkSlicerSRepRefinerLogic::ReorderCrestSpokes(int nRows, int nCols,
                                                                     std::vector<Spoke *> &input,
                                                                     std::vector<Spoke *> &output)
{
    vtkSmartPointer<vtkPoints> foldCurvePts = vtkSmartPointer<vtkPoints>::New();
    vtkSmartPointer<vtkCellArray> foldCurveCell = vtkSmartPointer<vtkCellArray>::New();
    // top row
...
void vtkSlicerSRepRefinerLogic::OptimizeCrestSpokeLength(vtkImplicitPolyDataDistance *distanceFunction,
                                                                           Spoke *targetSpoke, double stepSize, int maxIter)
{
    // 1. Transform the boundary point to image cs. by applying [x, y, z, 1] * mTransformationMat
    double pt[3];
