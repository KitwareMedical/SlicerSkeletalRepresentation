...
Spoke::Spoke(double *ptSkeletal, double *ptBoundary)
{
    mPx = ptSkeletal[0];
    mPy = ptSkeletal[1];
    mPz = ptSkeletal[2];
...
Spoke &Spoke::operator=(const Spoke &other)
{
    double u[3], p[3], r;
    other.GetDirection(u);
    other.GetSkeletalPoint(p);
...
void Spoke::SetRadius(double r)
{
    mR = r;
}

void Spoke::SetSkeletalPoint(double px, double py, double pz)
{
    mPx = px;
    mPy = py;
    mPz = pz;
...
void Spoke::SetDirection(double *u)
{
    vtkMath::Normalize(u);
    mUx = u[0];
    mUy = u[1];
...
void Spoke::GetDirection(double *output) const
{
    output[0] = this->mUx;
    output[1] = this->mUy;
    output[2] = this->mUz;
...
void Spoke::GetSkeletalPoint(double *output) const
{
    output[0] = mPx;
    output[1] = mPy;
    output[2] = mPz;
...
void Spoke::Add(Spoke *another, double *output) const
{
...
void Spoke::Diff(Spoke *another, double *output) const
{
    double eX = this->mR * this->mUx;
    double eY = this->mR * this->mUy;
    double eZ = this->mR * this->mUz;
...
void Spoke::GetBoundaryPoint(double *output) const
{
    output[0] = mPx + mR * mUx;
    output[1] = mPy + mR * mUy;
    output[2] = mPz + mR * mUz;
...
void Spoke::SetNeighborU(const std::vector<Spoke *> &neighbors, bool isForward)
{
    mNeighborsU.clear();
...
void Spoke::SetNeighborV(const std::vector<Spoke *> &neighbors, bool isForward)
{
    mNeighborsV.clear();
    for (size_t i = 0; i < neighbors.size(); ++i) {
...
double Spoke::GetRSradPenalty(double stepSize)
{
...
void Spoke::ComputeDerivatives(std::vector<Spoke*> neibors, bool isForward, double stepSize, // input
